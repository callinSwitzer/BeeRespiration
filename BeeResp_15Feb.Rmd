...
1.  First we see how $\Delta$load affects $\Delta$ArcLength and $\Delta$frequency^2, accounting for bee size and treatment order
...
# fit full model
m1 <- lm(deltaArcL ~  (deltaLoad +  size_pc1 + order_1)^2  + 
...
par(mfrow = c(2,2))
plot(m7, which = 1:4) # no glaring violations, though there are a few fairly influential observation
par(mfrow = c(1,1))
...
plot(x = newDF$deltaLoad, y = newDF$deltaArcL)
...
car::vif(m10) # vif is a little high, which
```
...
```{r}
mm1 <- lm(deltaMetR ~ deltaFrq2 + deltaArcL + deltaLoad + size_pc1 + deltaLoad2, data = newDF)
car::vif(mm1)

summary(mm1)

mm2 <- update(mm1, .~. - deltaArcL)
anova(mm1, mm2)
summary(mm2)
